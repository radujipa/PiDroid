...
public class RoboticPlatform {
    ...
    public RoboticPlatform(Messenger messenger) {
        this.motors = new MotorsController(messenger);
        this.camera = new CameraController(messenger);
    } // constructor
    ...
    public void notifyMotors(int power, Direction direction, State state) {
        ...
        if (power == 0 && state != State.STOPPED) {
            Log.e("notifyMotors():", "power is 0 but state != STOPPED!");
            return;
        }
    ...
    public void notifyMotors(int power) {
        if (power < 0 || power > 100) {
            Log.e("notifyMotors():", "power: either < 0 or > 100!");
            return;
        }
    ...
    public void notifyMotorsTurnAngle(int angle) {
        if (angle < 0 || angle > 180) {
            Log.e("notifyMotorsTurnAngle:", "angle: either < 0 or > 180!");
            return;
        }
    ...
    public void notifyCamera(int cameraX, int cameraY) {
        if (cameraX < 0 || cameraX > 100) {
            Log.e("notifyCamera():", "cameraX: either < 0 or > 100!");
            return;
        }
        if (cameraY < 0 || cameraY > 100) {
            Log.e("notifyCamera():", "cameraY: either < 0 or > 100!");
            return;
        }
